...
class Settings(BaseSettings):
    ...
    # 浏览器 CORS 需完整 Origin（含协议与端口），逗号分隔；勿与 TrustedHost 混用
    ...
    # 为 true 时，在 CORSMiddleware 中增加 allow_origin_regex，放行常见局域网 Origin（手机访问前端时无需逐条写 IP）
    CORS_ALLOW_LAN_REGEX: bool = os.getenv("CORS_ALLOW_LAN_REGEX", "true").lower() == "true"

    @property
    def cors_lan_origin_regex(self) -> str:
        """匹配 http(s)://localhost|127.0.0.1|192.168.x.x|10.x.x.x|172.16–31.x.x 及任意端口。"""
        return (
            r"https?://("
    ...
    def get_integration_configs(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "baidu_voice": {
                "name": "baidu_voice",
                "type": "voice",
                "enabled": bool(self.BAIDU_API_KEY and self.BAIDU_SECRET_KEY),
                "credentials": {
